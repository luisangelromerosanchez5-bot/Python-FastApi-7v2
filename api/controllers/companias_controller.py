...
from api.middlewares.security import get_current_user, require_admin, require_roles

router = APIRouter(prefix="/api/companias", tags=["Companias"])


@router.get("", response_model=list[CompaniaResponseDTO])
async def get_all(uow: IUnitOfWork = Depends(get_uow), _usuario=Depends(get_current_user)):
...
async def create_con_empleados(dto: CompaniaConEmpleadosCreateDTO, uow: IUnitOfWork = Depends(get_uow), _usuario=Depends(require_admin)):
...
async def create(dto: CompaniaCreateDTO, uow: IUnitOfWork = Depends(get_uow), _usuario=Depends(require_roles("ADMIN", "USUARIO"))):
    return await CompaniaService(uow).create(dto)


@router.put("/{id}", response_model=CompaniaResponseDTO)
async def update(id: int, dto: CompaniaUpdateDTO, uow: IUnitOfWork = Depends(get_uow), _usuario=Depends(require_roles("ADMIN", "USUARIO"))):
...
async def get_empleados(id: int, pagina: int = 1, tamano: int = 10, uow: IUnitOfWork = Depends(get_uow), _usuario=Depends(get_current_user)):
